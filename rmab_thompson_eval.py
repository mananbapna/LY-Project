#!/usr/bin/env python3
"""
rmab_thompson_eval.py

Reads outputs/rmab_thompson_learning.json and computes evaluation metrics:
  - belief accuracy vs TRUE_EFFICACY
  - cumulative regret and average regret per round
  - average success rate
  - average action accuracy
  - reward per cost

Run after: python3 rmab_thompson.py
"""

import json
from pathlib import Path
import statistics as stats

HISTORY_JSON = Path("outputs/rmab_thompson_learning.json")
EVAL_JSON = Path("outputs/rmab_thompson_eval.json")

# Must match the values in rmab_thompson.py
TRUE_EFFICACY = {
    "sms":         0.10,
    "assignment":  0.25,
    "mentor_call": 0.50,
}

RESOURCE_COSTS = {
    "sms": 1.0,
    "assignment": 5.0,
    "mentor_call": 10.0,
}


def main():
    if not HISTORY_JSON.exists():
        print(f"Error: {HISTORY_JSON} not found. Run rmab_thompson.py first.")
        return

    with open(HISTORY_JSON, "r") as f:
        history = json.load(f)

    if not history:
        print("History is empty; nothing to evaluate.")
        return

    n_rounds = len(history)

    # ---- Belief Accuracy (final round) ----
    final_beliefs = history[-1]["agent_beliefs"]
    belief_summary = []

    print("\n=== Belief vs True Efficacy (Final Round) ===")
    for res, true_eff in TRUE_EFFICACY.items():
        belief = final_beliefs.get(res, None)
        if belief is None:
            continue
        abs_err = abs(belief - true_eff)
        belief_acc = 1.0 - abs_err  # 1 - |est - true|
        belief_summary.append({
            "resource": res,
            "true_efficacy": true_eff,
            "learned_belief": belief,
            "abs_error": abs_err,
            "belief_accuracy": belief_acc,
        })
        print(
            f"{res:12s} | true={true_eff:.3f}  "
            f"belief={belief:.3f}  "
            f"| abs_err={abs_err:.3f}  "
            f"belief_acc={belief_acc:.3f}"
        )

    # ---- Regret ----
    final_cum_regret = history[-1]["cumulative_regret"]
    avg_regret_per_round = final_cum_regret / n_rounds

    # ---- Success metrics ----
    final_cum_successes = history[-1]["cumulative_successes"]
    final_cum_optimal = history[-1]["cumulative_optimal_successes"]

    policy_efficiency = (
        final_cum_successes / final_cum_optimal if final_cum_optimal > 0 else 0.0
    )

    success_rates = [h["success_rate"] for h in history]
    avg_success_rate = stats.mean(success_rates) if success_rates else 0.0

    # ---- Action accuracy ----
    action_accuracies = [h["action_accuracy"] for h in history]
    avg_action_accuracy = (
        stats.mean(action_accuracies) if action_accuracies else 0.0
    )

    # ---- Cost & reward-per-cost ----
    total_cost = sum(h.get("total_cost", 0.0) for h in history)
    reward_per_cost = (
        final_cum_successes / total_cost if total_cost > 0 else 0.0
    )

    print("\n=== Regret & Reward Metrics ===")
    print(f"Rounds                     : {n_rounds}")
    print(f"Final cumulative successes : {final_cum_successes:.2f}")
    print(f"Final cumulative optimal   : {final_cum_optimal:.2f}")
    print(f"Final cumulative regret    : {final_cum_regret:.2f}")
    print(f"Avg regret / round         : {avg_regret_per_round:.3f}")
    print(f"Policy efficiency (succ/opt): {policy_efficiency:.3f}")

    print("\n=== Rates & Accuracy ===")
    print(f"Average success rate       : {avg_success_rate:.3f}")
    print(f"Average action accuracy    : {avg_action_accuracy:.3f}")

    print("\n=== Cost Efficiency ===")
    print(f"Total cost spent           : {total_cost:.2f}")
    print(f"Reward per cost            : {reward_per_cost:.3f}")

    # Save compact JSON summary
    summary = {
        "rounds": n_rounds,
        "final_cumulative_successes": final_cum_successes,
        "final_cumulative_optimal_successes": final_cum_optimal,
        "final_cumulative_regret": final_cum_regret,
        "avg_regret_per_round": avg_regret_per_round,
        "policy_efficiency": policy_efficiency,
        "avg_success_rate": avg_success_rate,
        "avg_action_accuracy": avg_action_accuracy,
        "total_cost": total_cost,
        "reward_per_cost": reward_per_cost,
        "belief_summary": belief_summary,
    }

    EVAL_JSON.parent.mkdir(parents=True, exist_ok=True)
    with open(EVAL_JSON, "w") as f:
        json.dump(summary, f, indent=2)

    print(f"\nSaved evaluation summary to {EVAL_JSON}")


if __name__ == "__main__":
    main()
